...
def convert_to_mp4(
    ...
    *,
    ...
    with Path(filename + "_mesh.json").open("r") as f:
        ...
        triangles = np.array(
            [
                [[data["vertices"][i][0], data["vertices"][i][1]] for i in triangle]
    ...
    *,
    ...
    if plot_mesh:
        for triangle in triangle_list:
            fig.add_trace(
                go.Scatter(
                    x=(
                        [vertex_list[i][0] for i in triangle]
                        + [vertex_list[triangle[0]][0]]
                    ),
                    y=(
                        [vertex_list[i][1] for i in triangle]
    ...
    fig_quiv = ff.create_quiver(
        [
            (
                vertex_list[triangle[0]][0]
                + vertex_list[triangle[1]][0]
                + vertex_list[triangle[2]][0]
            )
            / 3
            for triangle in triangle_list
        ],
        [
            (
                vertex_list[triangle[0]][1]
                + vertex_list[triangle[1]][1]
                + vertex_list[triangle[2]][1]
            )
            / 3
            for triangle in triangle_list
        ],
...
def add_vector_field_plot(
    fig: FigureType,
    vertex_list: list[float],
    triangle_list: list[list[int]],
    vector_field: list[list[float]],
    color: list[int] = [255, 255, 255, 0.9],
) -> None:

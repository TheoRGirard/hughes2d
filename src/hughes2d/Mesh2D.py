...
class Mesh(object):
    ...
    def setExitsFromDomain(self, domain: NonConvexDomain) -> None:
        ...
        for i,edge in enumerate(self.edges):
            if(edge[0] in exitVertices and edge[1] in exitVertices):
                exitEdges.append(i)
        ...
        for index, edge in enumerate(self.edges):
                if( domain.hasWallPoint(self.vertices[edge[0]]) and domain.hasWallPoint(self.vertices[edge[1]]) and index not in exitEdges ) or ( domain.hasBoundaryPoint(self.vertices[edge[0]]) and domain.hasBoundaryPoint(self.vertices[edge[1]]) and index not in exitEdges):
    ...
    def computeTrianglesPerVertex(self) -> None:
        ...
        for index in range(len(self.vertices)):
            ...
            for triangleIndex, triangle in enumerate(self.triangles):
                if index in triangle:
                    Exclude = [P for P in triangle if P != index]
    ...
    def computeCellAreas(self) -> float:
        ...
        max = 0
        for triangle in self.triangles:
            A = self.triangles[triangle[0]]
            B = self.triangles[triangle[1]]
            C = self.triangles[triangle[2]]
    ...
    def appendDict(self, dico):
        DomainDico = {"type" : "Non-convex domain"}
        DomainDico["listOuterVertex"] = self.domain.listOuterVertex
        DomainDico["wallVertices"] = self.domain.wallVertices
        DomainDico["outerBoundary"] = self.domain.outerBoundary
        DomainDico["wallVertices"] = self.domain.wallVertices
        DomainDico["wallEdges"] = self.domain.wallEdges
        DomainDico["wallHolesPoint"] = self.domain.wallHolesPoint
        DomainDico["wallEdgeVertex "] = self.domain.wallEdgeVertex
        DomainDico["exitList"] = self.domain.exitList
        dico["domain"] = DomainDico

        MeshDico = {"type": "triangular mesh"}
        #MeshDico["dx"] = self.dx
        MeshDico["vertices"] = self.vertices.tolist()

        #MeshDico["meshDict"] = self.meshDict
        MeshDico["triangles"] = self.vertices.tolist()
        for i in range(len(MeshDico["triangles"])): #on passe tout en int au lieu de int32
            for j in range(3):
                MeshDico["triangles"][i][j] = int(MeshDico["triangles"][i][j])
        MeshDico["EdgeList"] = self.edges #Liste des edges par paires d'indices de vertex
        for i in range(len(MeshDico["EdgeList"])): #on passe tout en int au lieu de int32
            for j in range(len(MeshDico["EdgeList"][0])):
                MeshDico["EdgeList"][i][j] = int(MeshDico["EdgeList"][i][j])
        MeshDico["TriangleWithEdgeList"] = self.trianglesWithEdges #Liste des triangles par indice d'edge
        MeshDico["ExitEdges"] = self.ExitEdges #Liste des edges d'exit par indice d'edge
        MeshDico["WallEdges"] = self.WallEdges #Liste des wall edges par indice d'edge
        MeshDico["ListOfPairTriangles"] = self.pairsOfTriangles #Liste des triangles entourant les edges ordonnée comme la EdgeList par les indices des triangles.
        MeshDico["BoundaryEdgesIndex"] = self.boundaryEdgesIndex #Liste des indices des edges faisant le bord du domaine
        MeshDico["outerNormalVectByTriangles"] = self.outerNormalVectByTriangles #Liste de triplets correspondants aux 3 vecteurs normaux unitaires dans le même ordre que les edges correspondant dans la TriangleWithEdgeList
        MeshDico["cellAreas"] = self.cellAreas
        MeshDico["edgeLength"] = self.edgeLength
...
class CellValueMap(object):
    ...
    def __init__(self, Mesh):
        self.Mesh = Mesh
        self.values = [0 for _ in self.Mesh.meshDict['triangles']]

    def generateRandom(self,variability = 0.4):
        self.values = [ 0.2 + variability*(alea.random()-0.5) for _ in self.Mesh.meshDict['triangles']]
    ...
    def __add__(self, other):
        ...
        for index in range(len(self.Mesh.meshDict['triangles'])):
    ...
    def valueOnTriangle(self, L):
        for index, triangle in enumerate(self.Mesh.meshDict['triangles']):
    ...
    def getScatter(self):
        if(go):
            L = []
            for j,T in enumerate(self.Mesh.meshDict['triangles']):
                L.append(go.Scatter(x=[self.Mesh.meshDict['vertices'][i][0] for i in T]+[self.Mesh.meshDict['vertices'][T[0]][0]],
                                        y=[self.Mesh.meshDict['vertices'][i][1] for i in T]+[self.Mesh.meshDict['vertices'][T[0]][1]],
                                fill="toself",
                                hoverinfo = "none",
                                showlegend = False,
...
class VertexValueMap(object):
    ...
    def __init__(self, Mesh):
        self.Mesh = Mesh
        self.values = [0 for _ in self.Mesh.meshDict['vertices']]

    def generateRandom(self,variability = 0.5):
        self.values = [ 0.5 + variability*(alea.random()-0.5) for _ in self.Mesh.meshDict['vertices']]
    ...
    def setInfinity(self):
        self.values = [float('inf') for _ in self.Mesh.meshDict['vertices']]

    def checkGradientValidity(self):
        Grads = self.computeGradientFlow()
        for edgeIndex in self.Mesh.WallEdges:
            indexTriangle = self.Mesh.ListOfPairTriangles[edgeIndex][0]
            Grad = Grads[indexTriangle]
            for P in self.Mesh.meshDict['triangles'][indexTriangle]:
                if P not in self.Mesh.EdgeList[edgeIndex]:
                    OuterVect = ComputeOuterNormalUnitVect(self.Mesh.meshDict['vertices'][self.Mesh.EdgeList[edgeIndex][0]],
                                                            self.Mesh.meshDict['vertices'][self.Mesh.EdgeList[edgeIndex][1]],
                                                            self.Mesh.meshDict['vertices'][P])
                    C = P
            Scal = OuterVect[0]*Grad[0] + OuterVect[1]*Grad[1]
            if(Scal > 0):
                print("Vecteur non valide ! Emplacement du edge : ")
                print(self.Mesh.meshDict['vertices'][self.Mesh.EdgeList[edgeIndex][0]], self.Mesh.meshDict['vertices'][self.Mesh.EdgeList[edgeIndex][1]])
                print("Valeurs : ")
    ...
    def show3D(self):
        if(go):
            fig = go.Figure()
            fig.add_trace(go.Mesh3d(x=[P[0] for P in self.Mesh.meshDict['vertices']],
                                    y = [P[1] for P in self.Mesh.meshDict['vertices']],
                                    z = self.values,
                                    opacity=1,
    ...
    def show(self, grid=False, colorscale_name = 'viridis'):
        if(go):
            ...
            fig.add_trace(go.Scatter(x=[P[0] for P in self.Mesh.meshDict['vertices']],
                                    y = [P[1] for P in self.Mesh.meshDict['vertices']],
                            hoverinfo = "none",
                            showlegend = False,
                            mode="markers",
    ...
    def computeGradientFlow(self):
        if(go):
            LTrianglesGrad = []
            for point in range(len(self.Mesh.meshDict['vertices'])):
                if(self.values[point] < 0):
                    print("GROS PROBLEME");
            for triangle in self.Mesh.meshDict['triangles']: #On a des déterminants égaux à 0....
                    det =  ((self.Mesh.meshDict['vertices'][triangle[1]][0] - self.Mesh.meshDict['vertices'][triangle[0]][0])*(self.Mesh.meshDict['vertices'][triangle[2]][1] - self.Mesh.meshDict['vertices'][triangle[0]][1])
                            - (self.Mesh.meshDict['vertices'][triangle[2]][0] - self.Mesh.meshDict['vertices'][triangle[0]][0])*(self.Mesh.meshDict['vertices'][triangle[1]][1] - self.Mesh.meshDict['vertices'][triangle[0]][1]))
                    if(det == 0):
                        print("det nul :")
                        print(str(self.Mesh.meshDict['vertices'][triangle[0]][0])+','+str(self.Mesh.meshDict['vertices'][triangle[0]][1]))
                        print(str(self.Mesh.meshDict['vertices'][triangle[1]][0])+','+str(self.Mesh.meshDict['vertices'][triangle[1]][1]))
                        print(str(self.Mesh.meshDict['vertices'][triangle[2]][0])+','+str(self.Mesh.meshDict['vertices'][triangle[2]][1]))
                    Vecx = ( (self.values[triangle[0]] - self.values[triangle[2]])*(self.Mesh.meshDict['vertices'][triangle[1]][1] - self.Mesh.meshDict['vertices'][triangle[0]][1])
                            + (self.values[triangle[1]] - self.values[triangle[0]])*(self.Mesh.meshDict['vertices'][triangle[2]][1] - self.Mesh.meshDict['vertices'][triangle[0]][1]) )/det
                    Vecy = ( (self.values[triangle[0]] - self.values[triangle[1]])*(self.Mesh.meshDict['vertices'][triangle[2]][0] - self.Mesh.meshDict['vertices'][triangle[0]][0])
                            + (self.values[triangle[2]] - self.values[triangle[0]])*(self.Mesh.meshDict['vertices'][triangle[1]][0] - self.Mesh.meshDict['vertices'][triangle[0]][0]) )/det
    ...
    def showVectorField(self):
        if(go):
            ...
            figQuiv = ff.create_quiver([(self.Mesh.meshDict['vertices'][T[0]][0]+self.Mesh.meshDict['vertices'][T[1]][0]+self.Mesh.meshDict['vertices'][T[2]][0])/3 for T in self.Mesh.meshDict['triangles']],
                                        [(self.Mesh.meshDict['vertices'][T[0]][1]+self.Mesh.meshDict['vertices'][T[1]][1]+self.Mesh.meshDict['vertices'][T[2]][1])/3 for T in self.Mesh.meshDict['triangles']],

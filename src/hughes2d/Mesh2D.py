...
class NonConvexDomain:
    ...
    def __init__(
        self, outer_vertices_list: list[PointType] = PLACEHOLDER_DOMAIN
    ) -> None:
    ...
    def add_boundary_point(self, point: PointType) -> int:
        ...
        if point not in self.outer_vertices:
            for index, edge in enumerate(self.outer_boundary):
                if NonConvexDomain.belong_segment(
                    point, [self.outer_vertices[edge[i]] for i in [0, 1]]
                ):
                    id_point = len(self.outer_vertices)
                    self.outer_vertices.append(point)
                    self.outer_boundary = [
                        *(self.outer_boundary[:index]),
                        [edge[0], id_point],
                        [id_point, edge[1]],
        ...
        for i, point2 in enumerate(self.outer_vertices):
            if point2 == point:
                return i
        msg = (
            "_point " + str(point) + " detected inside the boundary but doesn't"
    ...
    def add_wall_point(self, point: PointType) -> int:
        ...
        if point not in self.wall_vertices:
            for index, edge in enumerate(self.wall_edges):
                if NonConvexDomain.belong_segment(
                    point, [self.wall_vertices[edge[i]] for i in [0, 1]]
                ):
                    id_point = len(self.wall_vertices)
                    self.wall_vertices.append(point)
                    self.wall_edges = [
                        *self.wall_edges[:index],
                        [edge[0], id_point],
                        [id_point, edge[1]],
        ...
        for i, point2 in enumerate(self.wall_vertices):
            if point2 == point:
                return i
        msg = (
            "_point " + str(point) + " detected inside an inner wall but doesn't"
            "correspond to any of the existing vertices."
        )
        raise ValueError(msg)
    ...
    def add_wall(self, coord_wall: list[PointType], *, cycle: bool = False) -> None:
        ...
        if cycle:  # CircularWall
            for i in range(
                len(self.wall_vertices), len(self.wall_vertices) + len(coord_wall) - 1
            ):
                self.wall_edges.append([i, i + 1])
            self.wall_edges.append(
                [len(self.wall_vertices) + len(coord_wall) - 1, len(self.wall_vertices)]
            )
            ...
            self.wall_holes_point.append(
                [center_point[0] / len(coord_wall), center_point[1] / len(coord_wall)]
            )
        else:
            for i in range(
                len(self.wall_vertices), len(self.wall_vertices) + len(coord_wall) - 1
    ...
    def add_exit(self, exit_edge: list[PointType]) -> None:
        ...
        if len(exit_edge) > 2:
            msg = (
                "Invalid value: "
                + str(exit_edge)
                + ", an exit is a list of two points."
            )
            raise ValueError(msg)
        if self.has_wall_edge(exit_edge):
            ...
            exit_path = NonConvexDomain.shortest_path_bfs(
                vertex_index[0], vertex_index[1], self.wall_edges
            )
            if len(exit_path) == 0:
            ...
            exit_edges = [
                [self.wall_vertices[exit_path[i]], self.wall_vertices[exit_path[i + 1]]]
            ...
            exit_path = NonConvexDomain.shortest_path_bfs(
                vertex_index[0], vertex_index[1], self.outer_boundary
            )
            if len(exit_path) == 0:
            ...
            exit_edges = [
                [
                    self.outer_vertices[exit_path[i]],
            ...
            msg = (
                "The exit " + str(exit_edge) + " is not inside an inner wall nor in"
                "an outer oboundary."
            )
            raise ValueError(msg)
    ...
    def find_boundary_point(self, point: PointType) -> int:
        ...
        for i, edge in enumerate(self.outer_boundary):
            if NonConvexDomain.belong_segment(
                point, [self.outer_vertices[edge[0]], self.outer_vertices[edge[1]]]
            ):
                return i
        return -1
    ...
    def find_wall_point(self, point: PointType) -> int:
        ...
        for i, edge in enumerate(self.wall_edges):
            if NonConvexDomain.belong_segment(
                point, [self.wall_vertices[edge[0]], self.wall_vertices[edge[1]]]
            ):
                return i
        return -1
    ...
    def has_exit_edge(self, edge: list[PointType]) -> bool:
        ...
        return (
            NonConvexDomain.belong_segment(
                self.exit_list[exit_indices[0]][0], self.exit_list[exit_indices[1]]
            )
            or NonConvexDomain.belong_segment(
                self.exit_list[exit_indices[0]][1], self.exit_list[exit_indices[1]]
            )
        ) and np.abs(
            (
                self.exit_list[exit_indices[0]][1][0]
    ...
    def has_wall_edge(self, edge: list[PointType]) -> bool:
        ...
        if wall_indices[0] == -1 or wall_indices[1] == -1:
            return False
        wall1_coord = [
            self.wall_vertices[self.wall_edges[wall_indices[0]][0]],
            self.wall_vertices[self.wall_edges[wall_indices[0]][1]],
        ]
        wall2_coord = [
            self.wall_vertices[self.wall_edges[wall_indices[1]][0]],
        ...
        if NonConvexDomain.belong_segment(
            ...
            return True
        if (
            NonConvexDomain.belong_segment(wall1_coord[0], wall2_coord)
            or NonConvexDomain.belong_segment(wall1_coord[1], wall2_coord)
        ) and np.abs(
            (wall1_coord[1][0] - wall1_coord[0][0])
        ...
        for index, point in enumerate(self.wall_vertices):
            for i in range(2):
                if (
                    (edge[i][0] - point[0]) ** 2 + (edge[i][1] - point[1]) ** 2
                ) < PRECISION:
                    vertices_index[i] = index
        for wall in self.wall_edges:
            if (wall[0] == vertices_index[0] and wall[1] == vertices_index[1]) or (
                wall[0] == vertices_index[1] and wall[1] == vertices_index[0]
            ):
                return True
    ...
    def has_outer_edge(self, edge: list[PointType]) -> bool:
        ...
        """
        outer_indices = [
            self.find_boundary_point(edge[0]),
            self.find_boundary_point(edge[1]),
        ]
        if outer_indices[0] == -1 or outer_indices[1] == -1:
            return False
        outer1_coord = [
            self.outer_vertices[self.outer_boundary[outer_indices[0]][0]],
            self.outer_vertices[self.outer_boundary[outer_indices[0]][1]],
        ]
        outer2_coord = [
            self.outer_vertices[self.outer_boundary[outer_indices[1]][0]],
        ...
        if NonConvexDomain.belong_segment(
            ...
            return True
        if (
            NonConvexDomain.belong_segment(outer1_coord[0], outer2_coord)
            or NonConvexDomain.belong_segment(outer1_coord[1], outer2_coord)
        ) and np.abs(
            (outer1_coord[1][0] - outer1_coord[0][0])
        ...
        for i in self.outer_boundary[outer_indices[0]]:
            for j in self.outer_boundary[outer_indices[1]]:
                if (
                    self.outer_vertices[i][0] == edge[0][0]
                    ...
                    for edge_test in self.outer_boundary:
                        if (edge_test[0] == i and edge_test[1] == j) or (
                            edge_test[1] == i and edge_test[0] == j
                        ):
                            return True
        return False
    ...
    def show(self, preference: str = "plotly") -> None:
        ...
        if go and (not plt or preference == "plotly"):
            ...
            msg = (
                "No plotting module found. Try installing plotly or matplotlib"
                " if you want to use show methods"
            )
            raise ImportError(msg)

    def add_plot(
        self, fig: FigureType, ax: AxesType = None, preference: str = "plotly"
    ) -> None:
        ...
        if go and (not plt or preference == "plotly"):  # plotly version of the plot
            ...
            fig.add_trace(
                go.Scatter(
                    x=[
                        *[P[0] for P in ordered_outer_vertices],
                        ordered_outer_vertices[0][0],
                    ],
                    y=[
                        *[P[1] for P in ordered_outer_vertices],
            ...
            domain_polygon = patches.Polygon(
                [(P[0], P[1]) for P in ordered_outer_vertices],
                edgecolor="black",
                facecolor="white",
            )
            ...
            for edge in self.wall_edges:
                path = [
                    (self.wall_vertices[edge[0]][0], self.wall_vertices[edge[0]][1]),
    ...
    def shortest_path_bfs(
        i_start: int, i_end: int, list_edges: list[list[int]]
    ) -> list[int]:
    ...
    def belong_segment(point: PointType, segment: list[PointType]) -> bool:
        ...
        if (
            np.abs(
                (point_a[0] - point[0]) * (point_a[1] - point_b[1])
            ...
            if (
                scal <= (point_a[0] - point_b[0]) ** 2 + (point_a[1] - point_b[1]) ** 2
                and scal >= 0
            ):
                return True
        return False
...
def belong_triangle(point: PointType, triangle: list[PointType]) -> bool:
    ...
    projection_x = (
        (point[0] - triangle[0][0]) * (triangle[2][1] - triangle[0][1])
        - (point[1] - triangle[0][1]) * (triangle[2][0] - triangle[0][0])
    ) / det
    projection_y = (
        (triangle[1][0] - triangle[0][0]) * (point[1] - triangle[0][1])
        - (triangle[1][1] - triangle[0][1]) * (point[0] - triangle[0][0])
    ) / det
...
class Mesh:
    ...
    def import_mesh_from_msh(
        ...
        *,
        ...
        if "line" in file_mesh.cells_dict:
            for index, edge in enumerate(self.edges):
                for outer_edge in file_mesh.cells_dict["line"]:
                    if (edge[0] == outer_edge[0] and edge[1] == outer_edge[1]) or (
                        edge[0] == outer_edge[1] and edge[1] == outer_edge[0]
                    ):
                        special_edges_index.append(index)
                        break
            if len(special_edges_index) != len(file_mesh.cells_dict["line"]):
                msg = (
                    "Error importing " + str(filename) + " : some line cells are not"
                    " edges from the mesh."
                )
                raise ImportError(msg)
        ...
        if "hughes2d:special" in file_mesh.cell_data:
            ...
            print(
                "WARNING : Mesh without walls imported from msh. Setting all special"
                " edges as walls."
            )
            wall_edges = [
                edge_index
                for edge_index in special_edges_index
        ...
        if (
            len(special_edges_index) == 0
            and len(self.exit_edges) == 0
            and len(self.wall_edges) == 0
        ):
            msg = (
                "Error importing "
                + str(filename)
                + " : Mesh without walls neither exits"
            )
            raise ImportError(msg)
    ...
    def import_mesh_from_msh_free_fem(
        self,
        filename: str,
        flag_dict: dict = DEFAULT_MESH_DICT,
        requirements: list[str] = DEFAULT_REQUIREMENTS,
        *,
        verbose: bool = False,
    ) -> None:
        ...
        with Path(filename).open("r") as file:
            ...
            nb_vertices, nb_triangles, nb_spe_edges = (
                int(line[0]),
                int(line[1]),
                int(line[2]),
            )
            ...
            for _ in range(nb_spe_edges):
                ...
                for index, edge in enumerate(self.edges):
                    if (
                        edge[0] == int(line[0]) - 1 and edge[1] == int(line[1]) - 1
                        ...
                        if int(line[2]) in flag_dict["exit"]:
                            ...
                            print(
                                "Warning : some special edges are neither wall nor exit"
                            )
                        break
            ...
            if len(self.exit_edges) == 0 and len(self.wall_edges) == 0:
                msg = (
                    "Error importing " + str(filename) + " : Mesh without"
                    " walls neither exits"
                )
                raise ImportError(msg)
            if nb_spe_edges != len(self.exit_edges) + len(self.wall_edges):
                print(
                    "WARNING : Losing some special edges during the import of .msh file."
                )
    ...
    def add_convex_zone(self, zone_name: str, zone_vertices: list[PointType]) -> None:
        ...
        self.zones[zone_name] = {"boundary": zone_vertices, "triangles": []}
    ...
    def in_zone(self, point: PointType, zone_name: str) -> bool:
        ...
        for i in range(1, len(self.zones[zone_name]["boundary"]) - 1):
            if belong_triangle(
                point,
                [
                    self.zones[zone_name]["boundary"][0],
    ...
    def export_mesh_msh(self, filename: str) -> None:
        ...
        cells = [
            (
                ...
                np.array(
                    [
                        self.edges[i]
                        for i in np.concatenate([self.exit_edges, self.wall_edges])
                    ]
                ),
            ),
        ...
        cell_data_dict = {
            "gmsh:geometrical": [
                [4 for i in np.concatenate([self.exit_edges, self.wall_edges])],
    ...
    def export_mesh_msh_free_fem(self, filename: str) -> None:
        ...
        with Path(filename).open("w") as file:
            ...
            file.writelines(
                [
                    f"{point[0]} {point[1]} {self.vertex_flags[i]}\n"
            ...
            file.writelines(
                [
                    f"{triangle[0] + 1} {triangle[1] + 1} {triangle[2] + 1} 0 \n"
    ...
    def generate_mesh_from_domain(
        ...
        *,
        ...
        for edge in domain.wall_edges:
            domain_special_edges.append(
                [edge[0] + len(domain_vertices), edge[1] + len(domain_vertices)]
            )
        ...
        *,
        ...
        if "all" in requirements:
            ...
            if "LWRSolver" in requirements or "integrate" in requirements:
                ...
                if verbose:
                    print(
                        "Minimal area for a triangle in the mesh : ", self.min_cell_area
                    )
        ...
        for triangle in self.triangles:
            ...
            for edge2 in sides:
                ...
                for n, edge in enumerate(edges):
                    if (edge[0] == edge2[0] and edge[1] == edge2[1]) or (
                        edge[0] == edge2[1] and edge[1] == edge2[0]
                    ):
                        is_already_in = 1
    ...
    def set_exits_from_domain(self, domain: NonConvexDomain) -> None:
        ...
        for index, edge in enumerate(self.edges):
            if (
                domain.has_wall_edge([self.vertices[edge[0]], self.vertices[edge[1]]])
                and not domain.has_exit_edge(
                    [self.vertices[edge[0]], self.vertices[edge[1]]]
                )
            ) or (
                domain.has_outer_edge([self.vertices[edge[0]], self.vertices[edge[1]]])
                and not domain.has_exit_edge(
                    [self.vertices[edge[0]], self.vertices[edge[1]]]
                )
            ):
    ...
    def compute_vertex_flags(
        self, domain: NonConvexDomain, flag_dict: dict = DEFAULT_MESH_DICT
    ) -> None:
    ...
    def compute_pair_of_triangles_list(self) -> None:
        ...
        for index in range(len(self.edges)):
            pair_of_triangles = []
            number_of_loop = 0
            while len(pair_of_triangles) < TWO and number_of_loop < len(
                self.triangles_with_edges
            ):
    ...
    def compute_outer_normals(self) -> None:
        ...
        def compute_outer_normal_unit_vect(
            a: PointType, b: PointType, c: PointType
        ) -> PointType:
        ...
        for index, triangle in enumerate(self.triangles_with_edges):
            normals = []
            normals = [
                compute_outer_normal_unit_vect(
            ...
            if len(normals) != 3:
                msg = (
                    f"Corrupted mesh lists, found {len(normals)} sides on one triangle."
                )
                raise ValueError(msg)
    ...
    def save_to_json(self, filename: str) -> None:
        ...
        mesh_dico["outerVertices"] = [
            self.vertices[i].tolist() for i in self.boundary_points
        ]
        ...
        mesh_dico["outerNormalVectByTriangles"] = (
            self.outer_normal_vect_by_triangles.tolist()
        )
    ...
    def load_from_json(self, filename: str) -> None:
        ...
        with Path(filename).open("r") as f:
            ...
            self.outer_normal_vect_by_triangles: ArrayLike = np.array(
                data["outerNormalVectByTriangles"]
            )
    ...
    def show(
        self, with_domain: NonConvexDomain = None, preference: str = "plotly"
    ) -> None:
        ...
        if go and (not plt or preference == "plotly"):
            ...
            for triangle in self.triangles:
                fig.add_trace(
                    go.Scatter(
                        x=(
                            [self.vertices[i][0] for i in triangle]
                            + [self.vertices[triangle[0]][0]]
                        ),
                        y=(
                            [self.vertices[i][1] for i in triangle]
            ...
            for edge in self.wall_edges:
                path = [
                    (
                        self.vertices[self.edges[edge][0]][0],
            ...
            for exit_edge in self.exit_edges:
                path = [
                    (
                        self.vertices[self.edges[exit_edge][0]][0],
    ...
    def add_plot(
        ...
        if go and (not plt or preference == "plotly"):
            ...
            for triangle in self.triangles:
                triangle_fig = patches.Polygon(
                    [(self.vertices[i][0], self.vertices[i][1]) for i in triangle],
                    edgecolor="black",
                    facecolor="white",
                )
...
class CellValueMap:
    ...
    def generate_random(self, variability: float = 0.4, mean: float = 0.23) -> None:
        ...
        """
        self.values = [
            mean + variability * (alea.random() - 0.5) for _ in self.mesh.triangles
        ]
    ...
    def __add__(self, other: Self | float) -> Self:
        ...
        msg = (
            "A CellValueMap can be summed only with another CellValueMap"
    ...
    def __mul__(self, other: Self | float) -> Self:
        ...
        msg = (
            "A CellValueMap can only be multiplied with another CellValueMap"
            " or a float or an integer."
        )
        raise TypeError(msg)
    ...
    def integrate(self) -> float:
        ...
        return sum(
            [
                self.values[i] * self.mesh.cell_areas[i]
                for i in range(len(self.mesh.triangles))
            ]
        )

    def set_constant_circle(
        self, center: list[PointType], radius: float, value: float
    ) -> None:
        ...
        for index in range(len(self.mesh.triangles)):
            if (
                (self.mesh.barycenters[index][0] - center[0]) ** 2
                + (self.mesh.barycenters[index][1] - center[1]) ** 2
            ) <= radius**2:
                self.values[index] = value

    def convolution_over_square_ball(
        self, radius: float, conv_func: Callable[[float], float]
    ) -> list:
        ...
        def recursive_integral(
            center: list[float], rad: float, visited: list[int], index: int
        ) -> float:
            ...
            if dist_x > rad or dist_y > rad:
                return 0.0
            conv_sum = (
                conv_func(self.values[index], dist_x, dist_y)
                * self.mesh.cell_areas[index]
            )
            for vertex in self.mesh.triangles[index]:
                for neighbor_triangle in self.mesh.triangles_per_vertex[vertex]:
                    if neighbor_triangle[0] not in visited:
                        conv_sum += recursive_integral(
                            center, rad, visited, neighbor_triangle[0]
                        )
            return conv_sum

        return [
            recursive_integral(
                self.mesh.vertices[vertexIndex],
                radius,
                [],
        ...
        if go and (not plt or preference == "plotly"):
            ...
            for j, triangle in enumerate(self.mesh.triangles):
                fig.add_trace(
                    go.Scatter(
                        x=(
                            [self.mesh.vertices[i][0] for i in triangle]
                            + [self.mesh.vertices[triangle[0]][0]]
                        ),
                        y=(
                            [self.mesh.vertices[i][1] for i in triangle]
            ...
            col = collections.PolyCollection(
                [
                    [
                        (self.mesh.vertices[i][0], self.mesh.vertices[i][1])
                        for i in triangle
                    ]
...
class VertexValueMap:
    ...
    def generate_random(self, variability: float = 0.5) -> None:
        ...
        self.values = np.array(
            [0.5 + variability * (alea.random() - 0.5) for _ in self.mesh.vertices]
        )
    ...
    def show_3d(self) -> None:
        ...
        if go:
            ...
            fig.add_trace(
                go.Mesh3d(
                    x=[P[0] for P in self.mesh.vertices],
            ...
            msg = (
                "Plotly is not installed. Try installing plotly "
                " if you want to use this show method"
            )
            raise ImportError(msg)

    def add_3d_plot(
        self, fig: FigureType, color: list[float] = DEFAULT_COLOR_RGBA
    ) -> None:
    ...
    def show(self, colorscale_name: str = "viridis", *, grid: bool = False) -> None:
        ...
        if go:
            fig = go.Figure()
            if grid:
                self.mesh.add_plot(fig, ax=None, preference="plotly")
            fig.add_trace(
                go.Scatter(
                    x=[P[0] for P in self.mesh.vertices],
                    y=[P[1] for P in self.mesh.vertices],
    ...
    def compute_gradient_flow(
        ...
        *,
        ...
        for triangle in self.mesh.triangles:
            det = (
                self.mesh.vertices[triangle[1]][0] - self.mesh.vertices[triangle[0]][0]
            ) * (
                self.mesh.vertices[triangle[2]][1] - self.mesh.vertices[triangle[0]][1]
            ) - (
                self.mesh.vertices[triangle[2]][0] - self.mesh.vertices[triangle[0]][0]
            ) * (
                self.mesh.vertices[triangle[1]][1] - self.mesh.vertices[triangle[0]][1]
            )
            if det == 0:
                msg = (
                    f"The triangle [{self.mesh.vertices[triangle[0]]},"
                    f"{self.mesh.vertices[triangle[1]]},"
                    f"{self.mesh.vertices[triangle[2]]}] is degenerated."
                )
                raise ValueError(msg)
            vec_x = (
                (self.values[triangle[0]] - self.values[triangle[2]])
                * (
            ...
            vec_y = (
                (self.values[triangle[0]] - self.values[triangle[1]])
                * (
            ...
            list_triangles_grads.append(
                [
                    -vec_x / normalization(vec_x, vec_y),
    ...
    def compute_vertex_gradient_flow(
        self,
        normalization: Callable[[float, float], float] = (
            lambda x, y: np.sqrt(x**2 + y**2)
        ),
            ...
            for i in range(len(self.mesh.triangles_per_vertex[vertex])):
                for point in self.mesh.triangles_per_vertex[vertex][i][1]:
                    if point not in treated_vertices:
                        ...
                        mean_vect += (
                            (self.values[point] - self.values[vertex])
                            / np.sqrt(
                                (
                                    self.mesh.vertices[point][0]
                                    - self.mesh.vertices[vertex][0]
                                )
                                ** 2
                                + (
                                    self.mesh.vertices[point][1]
                                    - self.mesh.vertices[vertex][1]
                                )
                                ** 2
                            )
    ...
    def show_vector_field(
        ...
        *,
        ...
        """
        if go:
            vector_field = self.compute_gradient_flow(

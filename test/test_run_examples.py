...
example_files_with_plot = [
    Path(example_folder, folder, file)
    for folder, file in [
...
@pytest.mark.parametrize("filename", example_files_with_plotly)
def test_run_plotly_examples(filename):
    if go:
        runpy.run_path(filename)
    else:
        with pytest.raises(ImportError):

...
ListModel = ["data/TestTPID0"] + [ "data/TestTPID1_"+str(i) for i in range(3,8)]

TM = np.arange(14,0,-0.1)
...
i = 0
#saveTimeSlices(T[i],filename=ListModel[i],slicename = "figs/testTP_ID"+str(i), limits=[[0,12],[0,7]])


compares = [[0,i] for i in range(1,len(ListModel))]
L1Diffs = [[] for _ in compares]
X = [[] for _ in compares]
PlotNames = ["Diff. Xc = "+str(L[1]+2)+" with Id1" for L in compares]
...
axs.legend()
#plt.show()
plt.savefig("figs/comparaisonID2-distance.png")
